...
int Torso::max_batteries() const {return _max_battery_slots;}

ostream& operator<<(ostream& os, const Torso& torso) {
  return os << fixed << setprecision(2)
            << torso.name() << " (" << torso.type() << " PN " << torso.part_number()
            << ") weighs " << torso.weight() << "#, costs $" << torso.cost()
            << ", " << torso.max_batteries() << " batteries max"
            << " - " << torso.description();
}
